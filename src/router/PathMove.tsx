...
export default function PathMove() {
    ...
    // <Link> : 현재 웹 애플리케이션 내에서 url을 변경할 수 있도록 하는 컴포넌트
    // - <Link> 컴포넌트는 새로운 요청을 보내지 않음
    // - 외부 페이지로 이동할 때는 <a> 써야함
    // - 컴포넌트 클릭 시에 다른 작업을 같이 수행한다면 해당 작업이 정상적으로 완료되지 않을 수 있다
    // - 특정 작업 결과에 따라 다른 경로로 이동하고 싶을 때, <Link> 컴포넌트 사용 불가 
    const onClick = () => {
        const flag = false;
        if (!flag) return;
    };
    
    // location 객체의 href값 변경으로 경로를 이동하면 새로운 요청을 보냄
    // 현재 웹 애플리케이션의 상태를 유지할 수 없음
    const onLocationHref = () => {
        window.location.href = 'http://localhost:3000/component';
    };

    // useNavigate() : navigator 함수 반환하는 react-router의 hook 함수
